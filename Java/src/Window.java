...
public class Window implements KeyListener {
	...
	void inverseKinematics3df() throws InterruptedException 
	{
		...
		degrees_3 = String.valueOf(dec.format((float) (B)).replaceAll(",", "."));
	...
	public void arduino() throws InterruptedException {
		...
		Serial.writeData(String.valueOf(degrees) + "D" + String.valueOf(degrees_1) + "A" + String.valueOf(degrees_2) + "D" + String.valueOf(degrees_3) + "W");
				...
				if(controller.getName().equals("Controller (Wireless Gamepad F710)")) {
					...
					if(controller.getAxisName(1).equals("X-axeln") ) {
						...
						if(!cancel) {
							...
							x = x + (controller.getRXAxisValue())*1;
							y = y - (controller.getRYAxisValue())*1;

						}

						try {
							inverseKinematics3df();
